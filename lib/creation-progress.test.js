...
describe("stepsFor", () => {
  const ids = (mode) => stepsFor(mode).map((step) => step.id);

  it("shows only the steps of each flow", () => {
    expect(ids("create")).toEqual(["image", "pull", "build", "container", "start"]);
    expect(ids("edit")).toEqual(["removing", "image", "pull", "build", "container", "start"]);
    expect(ids("update")).toEqual([
      "image", "pull", "build", "backup", "stopping", "container", "start", "verify",
    ]);
  });

  it("orders the update steps", () => {
    expect(stepStatus("backup", "stopping")).toBe("done");
    expect(stepStatus("verify", "start")).toBe("pending");
  });
});
