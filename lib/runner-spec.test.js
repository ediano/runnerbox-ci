...
describe("buildContainerConfig", () => {
  it("monta o env do GitHub em escopo de repositório", () => {
    ...
    expect(config.Image).toBe("myoung34/docker-github-actions-runner:latest");
    expect(config.Env).toContain("RUNNER_SCOPE=repo");
    expect(config.Env).toContain("REPO_URL=https://github.com/acme/app");
    expect(config.Env).toContain("RUNNER_TOKEN=AAAA1111");
  ...
  it("monta o env do GitLab", () => {
    ...
    expect(config.Env).toContain("REGISTRATION_TOKEN=GR1348941abc");
    expect(config.Env).toContain("RUNNER_EXECUTOR=docker");
  });

  it("injeta o registro como Entrypoint na imagem oficial do GitLab", () => {
    const config = buildContainerConfig(gitlabInput);
    expect(config.Entrypoint[0]).toBe("/bin/sh");
    expect(config.Entrypoint[2]).toContain("gitlab-runner register --non-interactive");
    expect(config.Entrypoint[2]).toContain('--registration-token "$REGISTRATION_TOKEN"');
    expect(config.Entrypoint[2]).toContain("exec gitlab-runner run");
  });

  it("usa --token no Entrypoint quando o token é um authentication token", () => {
    const config = buildContainerConfig({ ...gitlabInput, token: "glrt-abc" });
    expect(config.Entrypoint[2]).toContain('--token "$REGISTRATION_TOKEN"');
  });

  it("não injeta Entrypoint quando a imagem base própria é usada", () => {
    process.env.RUNNERBOX_GITLAB_IMAGE = "runnerbox/gitlab-runner:latest";
