...
// A ordem das etapas é a ordem real do backend (lib/docker.js + lib/images.js). Os
// três fluxos compartilham a lista; `modes` diz em quais cada etapa aparece.
export const STEPS = [
  { id: "removing", label: "Removing the current container", modes: ["edit"] },
  { id: "image", label: "Preparing image" },
  { id: "pull", label: "Downloading base image" },
  { id: "build", label: "Building image" },
  { id: "backup", label: "Saving the runner registration", modes: ["update"] },
  { id: "stopping", label: "Stopping the current container", modes: ["update"] },
  { id: "container", label: "Creating container" },
  { id: "start", label: "Starting runner" },
  { id: "verify", label: "Checking that the runner stays up", modes: ["update"] },
];

export function stepsFor(mode) {
  return STEPS.filter((step) => !step.modes || step.modes.includes(mode));
}
