...
const CONTAINER_DATA_DIR = "/var/lib/runnerbox";

let resolvedDir;

// Dentro da imagem do painel usamos /var/lib/runnerbox. Em `npm run dev` o processo
// roda como um usuário comum, que não pode criar diretório em /var/lib — nesse caso
// caímos para .runnerbox no projeto, em vez de estourar EACCES na criação do runner.
function dataDir() {
  if (process.env.RUNNERBOX_DATA_DIR) return process.env.RUNNERBOX_DATA_DIR;
  if (resolvedDir) return resolvedDir;

  try {
    fs.mkdirSync(CONTAINER_DATA_DIR, { recursive: true, mode: 0o700 });
    resolvedDir = CONTAINER_DATA_DIR;
  } catch {
    resolvedDir = path.join(process.cwd(), ".runnerbox");
  }
  return resolvedDir;
}
