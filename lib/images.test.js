...
describe("ensureRunnerImage with refresh", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
  });

  it("rebuilds even when the image already exists", async () => {
    const docker = fakeDocker({ imageExists: true });
    await ensureRunnerImage(docker, "github", () => {}, { refresh: true });
    expect(docker.buildCalls).toHaveLength(1);
    expect(docker.buildCalls[0].opts.pull).toBe("true");
  });

  it("pulls an explicitly configured image instead of building it", async () => {
    vi.stubEnv("RUNNERBOX_GITHUB_IMAGE", "registry.local/runner:pinned");
    const docker = fakeDocker({ imageExists: true });
    await expect(ensureRunnerImage(docker, "github", () => {}, { refresh: true })).resolves.toBe(
      "registry.local/runner:pinned"
    );
    expect(docker.pullCalls).toEqual(["registry.local/runner:pinned"]);
    expect(docker.buildCalls).toHaveLength(0);
  });
});
