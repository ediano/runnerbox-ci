...
// `refresh` é o caminho da atualização: em vez de aceitar a imagem que já existe,
// busca a versão mais nova — pull para uma imagem de registry, rebuild com pull da
// base para a nossa. Sem ele, só constrói o que falta.
export async function ensureRunnerImage(docker, platform, onProgress = () => {}, { refresh = false } = {}) {
  ...
  if (refresh && process.env[IMAGE_ENV[platform]]) {
    onProgress({ type: "phase", phase: "image", message: `Pulling image ${image}…` });
    try {
      await followStream(docker, await docker.pull(image), onProgress);
      return image;
    } catch (err) {
      // Uma tag só local (ex.: build de desenvolvimento) não existe no registry: nesse
      // caso caímos para o build pelo contexto, como na criação.
      onProgress({ type: "log", message: `Pull failed (${err.message}); building from the local context.` });
    }
  }
  ...
  await followStream(docker, stream, onProgress);

  return image;
}

function followStream(docker, stream, onProgress) {
  return new Promise((resolve, reject) => {
    docker.modem.followProgress(
      ...
      // progresso em vez de uma tela parada. Um `docker pull` só emite a segunda.
      (event) => {
